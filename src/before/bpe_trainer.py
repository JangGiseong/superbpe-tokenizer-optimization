...
from typing import Dict, Tuple, List, Iterator, Optional, Callable
...
def _chunk_worker(args):
    ...
    """
    docs_batch, langs_batch, stage_val, max_doc_len = args

    from pretokenizer import Pretokenizer, Stage

    stage = Stage.STAGE1 if stage_val == 1 else Stage.STAGE2
    pretok = Pretokenizer(stage=stage, use_mecab=True)
...
def build_chunk_counts(
    ...
    """
    코퍼스 → 청크 빈도 딕셔너리 (병렬 처리).
    n_workers: None이면 CPU 코어 수 자동 감지 (Windows는 물리 코어 수 권장)
    """
    if n_workers is None:
        n_workers = max(1, multiprocessing.cpu_count() - 1)

    # 텍스트와 언어 코드를 미리 목록으로 변환
    texts_list = list(texts)
    langs_list = [lang_fn(d) if lang_fn else "en" for d in texts_list]

    # 배치 분할
    batch_size = max(1, len(texts_list) // n_workers)
    batches = []
    for i in range(0, len(texts_list), batch_size):
        batches.append(
            (
                texts_list[i : i + batch_size],
                langs_list[i : i + batch_size],
                pretokenizer.stage.value,  # int → picklable
                max_doc_len,
            )
        )

    print(
        f"  병렬 청크 구축: {n_workers}개 워커, {len(batches)}개 배치, "
        f"총 {len(texts_list):,}개 문서"
    )
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        for idx, local in enumerate(executor.map(_chunk_worker, batches)):
            for k, v in local.items():
                merged[k] += v
            elapsed = time.time() - t0
            print(
                f"  배치 {idx+1}/{len(batches)} 완료 [{elapsed:.0f}s] "
                f"고유 청크: {len(merged):,}",
                end="\r",
            )
